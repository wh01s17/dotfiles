...
Panel {
  ...
  BarIconButton {
    ...
    text: root.icon
    tooltipText: {
      if (!root.adapter) return "Bluetooth no disponible"
      if (!root.adapter.enabled) return "Bluetooth apagado"
      var connected = root.connectedDevices || []
      if (connected.length === 0) return "Bluetooth encendido · sin dispositivos"
      var names = []
      for (var i = 0; i < connected.length; i++)
        names.push(Model.deviceLabel(connected[i]) || "Dispositivo")
      return "Conectado · " + names.join("\n")
    }
