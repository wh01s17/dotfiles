...
Panel {
  ...
  BarIconButton {
    ...
    tooltipText: {
      if (root.kind === "wifi") {
        var ssid = root.connectedWifiNetwork ? String(root.connectedWifiNetwork.name || "") : ""
        return "Wi-Fi · " + (ssid || "Red desconocida")
          + (root.signalStrength >= 0 ? "\nSeñal · " + root.signalStrength + "%" : "")
      }
      if (root.kind === "ethernet") return "Ethernet · conectado"
      return "Sin conexión"
    }
