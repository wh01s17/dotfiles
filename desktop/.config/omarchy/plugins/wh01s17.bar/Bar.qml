...
Item {
  ...
  // Compatibility value for widgets using the old global API. The actual
  // hover reveal state lives on each BarPanel so one monitor never relayouts
  // the bars on the other outputs.
  ...
  function centerSectionRevealHeldFor(target) {
    var window = root.targetWindow(target)
    return !!window && window.centerSectionRevealHeld === true
  }
  ...
  component BarPanel: PanelWindow {
    ...
    // These properties and their timer are deliberately per PanelWindow.
    // The Bar root is shared by every output, so storing hover state there
    // makes a single pointer movement invalidate and relayout all monitors.
    property bool centerSectionHovered: false
    property bool barHovered: false
    property bool centerSectionRevealHeld: false

    // Revealing the indicators widens the center section. Keep it open while
    // the pointer remains anywhere on this bar to avoid a reveal/collapse loop
    // when the newly visible icons shift a neighbour under the pointer.
    ...
    Timer {
      ...
      interval: 120
      onTriggered: {
        if (!barWindow.centerSectionHovered && !barWindow.barHovered)
          barWindow.centerSectionRevealHeld = false
      }
    }
    ...
    Loader {
      ...
      HoverHandler {
        onHoveredChanged: barWindow.setBarHovered(hovered)
        ...
        Component.onDestruction: if (hovered) barWindow.setBarHovered(false)
      }
    }
  ...
  component CenterModules: Item {
    ...
    required property var surfaceWindow
    ...
    Component {
      ...
      Item {
        ...
        HoverHandler {
          onHoveredChanged: centerRoot.surfaceWindow.setCenterSectionHovered(hovered)
          Component.onDestruction: if (hovered) centerRoot.surfaceWindow.setCenterSectionHovered(false)
        }
  ...
  component ModuleSlot: Item {
    ...
    function injectProps() {
      ...
      if (!target) return
      if ("bar" in target) target.bar = firstParty || moduleName === "wh01s17.indicators"
