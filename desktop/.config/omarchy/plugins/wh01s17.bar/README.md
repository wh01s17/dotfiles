...
| `omarchy.network` | Wi-Fi/Ethernet icon + popup with Wi-Fi scan (manual rescan button), signal, connect, DNS provider selection | left = popup |
...
| `omarchy.bluetooth` | Bluetooth icon + popup with device list, rescan button (`r`), connect/disconnect, battery | left = popup · right = toggle radio |
