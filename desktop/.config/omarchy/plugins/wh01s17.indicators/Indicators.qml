...
BarWidget {
  ...
  readonly property bool centerRevealHeld: {
    if (!bar) return false
    if (typeof bar.centerSectionRevealHeldFor === "function")
      return bar.centerSectionRevealHeldFor(root)
    return bar.centerSectionRevealHeld === true
  }
  readonly property bool revealInactiveIndicators: alwaysShowIndicators || indicatorAreaHovered || indicatorItemHovered || (centerRevealHeld && bar.centerHoverRevealSuppressed !== true)
