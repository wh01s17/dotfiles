...
Panel {
  ...
  BarIconButton {
    ...
    tooltipText: {
      if (!root.hasOutput) return "Sin salida de audio"
      var level = root.outputMuted
        ? "Silenciado"
        : "Volumen · " + Math.round(root.outputVolume * 100) + "%"
      var device = root.nodeLabel(root.volumeSink)
      return device ? level + "\n" + device : level
    }
